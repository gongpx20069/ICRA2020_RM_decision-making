...
class RMEnv(gym.Env):
    ...
    def __init__(self):
        self.robot = Robot(50, 50)
        # self.enemyrobot = Robot(50, 300, color=(1, 0, 0), team='red')
        self.enemyrobot = Robot(760, 400, rotation=math.pi*3/2,color=(1, 0, 0), team='red')
    ...
    def get_state(self):
        return (*self.robot.get_state(), *self.enemyrobot.get_state(),*self.order_list)

    def step(self, action):
        self.start += 1
        assert self.action_space.contains(action), "%r (%s) invalid" % (action, type(action))

        r1 = self.robot.move(action[0], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                             robot_list=[self.enemyrobot], viewer=self.viewer)
        r2 = self.enemyrobot.move(action[1], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                                  robot_list=[self.robot], viewer=self.viewer)

        # r1+=self.distance(self.robot.x, self.robot.y)
        # r2+=self.distance(self.enemyrobot.x,self.enemyrobot.y)

        self.state = self.get_state()

        # self.time = (datetime.datetime.now() - self.start_time).seconds
        # if self.time>0 and self.time % 60 == 0:
        #     self.random_buff()

        # if self.start % 100 ==0:
        #     if self.robot.hp>self.enemyrobot.hp:
        #         r1 += 100
        #         r2 -= 100
        #     elif self.robot.hp<self.enemyrobot.hp:
        #         r1 -= 100
        #         r2 += 100
        #     else:
        #         r1 -=10
        ...
        if self.start>=FRAME_PER_SECOND*60*3 or self.robot.hp<=0 or self.enemyrobot.hp<=0:
    ...
    def reset(self):
        ...
        self.robot.reset(50, 50, self.viewer)
        # self.enemyrobot.reset(50, 300, self.viewer)
        self.enemyrobot.reset(760, 400, self.viewer)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            self.robot.is_render = True
            self.enemyrobot.is_render = True
            self.viewer = rendering.Viewer(screen_width, screen_height)

            if self.state is None:
                return None
            self.robot.render_robot(rendering, self.viewer)
            self.enemyrobot.render_robot(rendering, self.viewer)
            self.render_wall(rendering, self.viewer)
            self.render_buff(rendering, self.viewer)

        self.render_hp_bullet(self.robot.hp, self.robot.bullet_num,self.enemyrobot.hp, self.enemyrobot.bullet_num,rendering,self.viewer)
        # self.render_red_hp_bullet(self.enemyrobot.hp, rendering, self.viewer)
        self.robot.render_update_robot(rendering, self.viewer)
        self.enemyrobot.render_update_robot(rendering, self.viewer)
