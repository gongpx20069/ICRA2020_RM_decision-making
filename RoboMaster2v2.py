...
class RMEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.bluerobot2 = Robot(*BR2_POS,rotation=math.pi*3/2)
        self.redrobot1 = Robot(*RR1_POS, color=(1, 0, 0), team='red')
        self.redrobot2 = Robot(*RR2_POS, rotation=math.pi*3/2,color=(1, 0, 0), team='red')
        self.action_space = spaces.Tuple((spaces.Discrete(6), spaces.Discrete(6),spaces.Discrete(6),spaces.Discrete(6)))
        self.seed()
        self.viewer = None
        self.state = None
        map = Map()
    ...
    def get_state(self):
        return (*self.bluerobot1.get_state(), *self.bluerobot2.get_state(), *self.redrobot1.get_state(), *self.redrobot2.get_state(),*self.order_list)
    ...
    def step(self, action):
        ...
        r1 = self.bluerobot1.move(action[0], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                             robot_list=[self.bluerobot2,self.redrobot1,self.redrobot2], viewer=self.viewer)


        r2 = self.bluerobot2.move(action[1], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                                  robot_list=[self.bluerobot1,self.redrobot1,self.redrobot2], viewer=self.viewer)

        r3 = self.redrobot1.move(action[2], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                                  robot_list=[self.bluerobot2,self.bluerobot1,self.redrobot2], viewer=self.viewer)

        r4 = self.redrobot2.move(action[3], map=self.map, costmap=self.costmap, start=self.start, buff_list=self.buff_list,
                                  robot_list=[self.bluerobot2,self.bluerobot1,self.redrobot1], viewer=self.viewer)

        self.state = self.get_state()

        # if self.start % 100 ==0:
        #     if (self.bluerobot1.hp + self.bluerobot2.hp )>(self.redrobot1.hp + self.redrobot2.hp):
        #         r1 += 100
        #         r2 += 100
        #         r3 -= 100
        #         r4 -= 100
        #     elif (self.bluerobot1.hp + self.bluerobot2.hp )<(self.redrobot1.hp + self.redrobot2.hp):
        #         r1 -= 100
        #         r2 -= 100
        #         r3 += 100
        #         r4 += 100
        ...
        if self.start>=FRAME_PER_SECOND*60*3 or (self.bluerobot1.hp<=0 and self.bluerobot2.hp<=0 ) or (self.redrobot1.hp<=0 and self.redrobot2.hp<=0 ):
    ...
    def reset(self):
        ...
        self.bluerobot2.reset(*BR2_POS, self.viewer)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            self.bluerobot2.is_render = True
            ...
            self.bluerobot2.render_robot(rendering, self.viewer)
        ...
        self.bluerobot2.render_update_robot(rendering, self.viewer)
