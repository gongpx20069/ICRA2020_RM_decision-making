...
def wsm_state(ob):
    red1wsm = WSM(ob[8],ob[9])
    red2wsm = WSM(ob[13],ob[14])
    if red1wsm > red2wsm:
        return 1
    else:
        return 0
...
class ActorCritic1v2(nn.Module):
    def __init__(self, state_dim, n_latent_var=1024,fsmstate_dim=5):
        super(ActorCritic1v2, self).__init__()
...
if __name__ == '__main__':
    network = ActorCritic1v2(state_dim=33,fsmstate_dim=2)

    if device.type == 'cuda':
        network.cuda()
    adam = torch.optim.Adam(network.parameters(), lr=0.000001)
    if os.path.exists('./PPO_blue_HDFSM1v2.pth'):
        network.load_state_dict(torch.load('./PPO_blue_HDFSM1v2.pth', map_location='cpu'))
    ...
    for i in range(1000):
        ...
        if i % 20 == 0:
            ...
            torch.save(network.state_dict(), './PPO_blue_HDFSM1v2.pth')
